...
//    seacrh-button-start
    ...
    <tr>
        ...
        $tampil = mysqli_query($koneksi, "SELECT insiden.no_insiden,siswa.nama,siswa.kelas,insiden.perihal,insiden.walas,insiden.WK,insiden.tgl,insiden.lokasi
                                          FROM insiden
        ...
        while ($row = mysqli_fetch_array($tampil)) {
            ...
            $tgl = $row['tgl'];
            $lok = $row['lokasi'];
            $pvc = "<form method='post'>
                    <input type='hidden' name='ids' value='$nois' > 
                    <input type='hidden' name='stat' value='Sudah diperbaiki'>
                    <button class='btn btn-success' name='ceklis'><i class=\"fas fa-check\"></i></button>
                    </form>";
            if (isset($_POST['ceklis']))
            {
                $isd = $_POST['ids'];
                $statu = $_POST['stat'];
                $koneksi-> query("update insiden set status='$statu' where no_insiden = '$isd'");
